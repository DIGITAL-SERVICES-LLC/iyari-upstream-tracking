...
IYARI includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into IYARI and does not require a IYARI observability plugin. IYARI remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
IYARI execution remains available, while Relay scopes, middleware, plugins,
...
> [!WARNING]
> This removes the IYARI `observability/nemo_relay` plugin. Existing users
...
> longer activate exporters. Without the new variable, IYARI does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

IYARI requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
IYARI installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the IYARI process so configured interceptors can
...
Collection remains off unless IYARI policy enables it:
...
variable is unset, IYARI does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, IYARI
...
Relay plugin configuration is a process-level deployment choice, not a IYARI
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that IYARI process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, IYARI logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this IYARI process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the IYARI
process and share the same policy boundary.

Run profiles in separate IYARI processes when they require different trust
...
IYARI core owns one Relay host and one isolated Relay session scope per IYARI
...
marks do not require IYARI plugin registration. Shared-metrics marks must
...
```text
IYARI turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> IYARI shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

IYARI sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that IYARI used for the logical call, such as
...
counter in any rolling 24-hour window. The metric has no dimensions; IYARI
version, OS family, architecture, and install method remain bounded package
resources. Concurrent IYARI processes share the SQLite latch, so simultaneous
...
the owning IYARI session. The start counter contains only bounded execution
...
provider attempts for the same IYARI API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their IYARI tool-call
...
cancellations. Active task ownership follows the task ID if IYARI rotates its
...
outcome, approval outcome, latency, and explicit retry-count buckets. IYARI
...
tool or plugin names. IYARI does not infer retries from repeated tool names or
...
state, and a use-count bucket. IYARI derives reuse and patch-generation
...
are written with atomic replacement. Each package records the IYARI version,
...
The install identity is scoped to one `HERMES_HOME`. To reset it, stop IYARI
...
Run a real IYARI CLI turn against the deterministic local model server:
