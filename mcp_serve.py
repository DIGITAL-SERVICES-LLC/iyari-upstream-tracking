"""
IYARI MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (IYARI-specific extra)
...
class EventBridge:
    ...
    This is the IYARI equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the IYARI MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "IYARI messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the IYARI MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
