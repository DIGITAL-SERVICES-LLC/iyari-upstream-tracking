...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory IYARI was already launched from (for example ``hermes-agent``
    ...
    # Common recovery for config values like ``hermes-agent`` when IYARI was
...
# IYARI-internal env vars that should NOT leak into terminal subprocesses.
_HERMES_PROVIDER_ENV_FORCE_PREFIX = "_HERMES_FORCE_"

# IYARI-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a IYARI inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # IYARI-managed inference credential — Claude subscription auth is not a
    # working IYARI provider path.  Stripping it broke agent-spawned
...
# project's dependencies into the IYARI venv path instead of the project's own
# ``.venv`` — silently clobbering the IYARI environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# IYARI venv stays reachable via PATH (its bin dir is first), so stripping
...
# system/venv Pythons — to the IYARI venv's stdlib, which crashes with
# version-mismatch errors before a child script even imports a package
# (#75018). IYARI itself treats PYTHONHOME as contamination in its own
...
# _strip_hermes_owned_pythonpath() which removes only IYARI-owned entries,
# preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_hermes_internal_secret(key: str) -> bool:
    """Return True for IYARI-internal secrets injected under *dynamic* names.
    ...
    This is the single source of truth for "IYARI-internal dynamic secret"
...
def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local IYARI home override into subprocess env."""
    try:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter IYARI-managed secrets from a subprocess environment."""
    try:
    ...
    # the separate IYARI runtime venv.  The filter validates that relationship
...
# legitimate child IYARI spawns needs them, and they are the highest-value
...
def hermes_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child IYARI spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_HERMES_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _plugin_terminal_env_strip_keys():
        env.pop(key, None)
    # Internal routing hints and IYARI-internal dynamic secrets
...
def build_subprocess_env(
    ...
    *,
    ...
    IYARI secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local IYARI home override into ``HERMES_HOME`` and apply the
...
def _find_bash() -> str:
    ...
    raise RuntimeError(
        "Git Bash not found. IYARI requires Git for Windows on Windows.\n"
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart IYARI. If the override is blocked or later re-applied, "
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing IYARI-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes IYARI installs for
    itself, so on a machine where IYARI provisioned its own toolchain a
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    IYARI-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  IYARI
...
def _build_hermes_repo_root_aliases(
    ...
    """Return exact repo-root spellings emitted by IYARI launchers.
    ...
    without treating arbitrary descendants of HERMES_HOME as IYARI-owned.
    ...
    # strict resolve before accepting it as IYARI-owned.  Fail-closed: a
...
# --- IYARI venv / repo-root detection (module-level, computed once) ---

#: The IYARI repository root - three levels up from this file
...
#: etc.  Subprocesses that are NOT the IYARI backend don't need it and it
#: can shadow local packages.
_hermes_repo_root: Path = Path(__file__).resolve().parents[2]

#: Alternate spellings of the repo root that IYARI launchers may emit.
#: ``Path(__file__).resolve()`` canonicalizes symlinks/junctions, but the
#: Windows gateway launcher deliberately renders IYARI-owned paths under
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    ``<IYARI repo>/venv`` layout and a real venv marker; require both before
...
def _get_hermes_site_packages(env: dict) -> list[Path]:
    """Return exact site-packages dirs owned by the IYARI runtime.
...
def _strip_hermes_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip IYARI-owned PYTHONPATH entries, then the runtime marker vars.
...
def _strip_hermes_owned_pythonpath(env: dict) -> None:
    """Remove IYARI-owned PYTHONPATH entries from subprocess environments.

    Launchers prepend the IYARI repo root and the IYARI venv's
    ...
    user entries, so only entries proven IYARI-owned are removed:
    ...
    for entry in pp.split(os.pathsep):
        ...
        # Preserve raw spelling unless the exact component is IYARI-owned.
        if entry == "":
            kept.append(entry)
            continue
        ...
        # --- Check 1: IYARI venv site-packages ---
    ...
    if stripped:
        logger.debug(
            "Stripped IYARI-owned entries from PYTHONPATH: %s",
            stripped,
        )
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, IYARI trusts them.
    """
