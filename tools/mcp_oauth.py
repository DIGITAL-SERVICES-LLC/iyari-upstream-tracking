...
URL of IYARI's published Client ID Metadata Document (CIMD) as the
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "IYARI"
...
def _cached_redirect_port(storage: "HermesTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If IYARI restarts and chooses a
...
class HermesTokenStorage:
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # IYARI records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to IYARI.</p></body></html>"
            ) if code else (
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"IYARI' Client ID Metadata Document ({cimd_url}); set "
...
# 2026-07-28. The SDK does the protocol work; IYARI only decides whether a
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port IYARI picks
...
def _server_declined_cimd(storage: "HermesTokenStorage | None") -> bool:
    ...
    IYARI has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    ...
    Every early return below is a case where the redirect URI IYARI would
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "IYARI" / "IYARI" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "IYARI")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. IYARI is a CLI/desktop app
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. IYARI auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older IYARI is running.
    """
    msg = str(exc)
    lowered = msg.lower()
    ...
    if _is_figma_remote_mcp(server_name, server_url):
        return (
            ...
            "work; most other names 403). IYARI defaults to "
