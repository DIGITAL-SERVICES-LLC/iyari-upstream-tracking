#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the IYARI Skills Hub.
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Digital Services LLC) but not activated
    ...
    OFFICIAL_REPO = "digital-services-llc/iyari"
...
# ---------------------------------------------------------------------------
# IYARI centralized index source
# ---------------------------------------------------------------------------
...
def _load_hermes_index() -> Optional[dict]:
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("IYARI index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "IYARI index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("IYARI index fetch failed: %s", e)
            return _load_stale_index_cache()
...
class HermesIndexSource(SkillSource):
    """Skill source backed by the centralized IYARI Skills Index.
