...
"""
    ...
    assert "Tyler Williams > Active Projects: IYARI" in entries
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``IYARI``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "IYARI prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told IYARI to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "IYARI config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "IYARI uses tools well"
    # All-lowercase matches → lowercase ``hermes``; this preserves the
    # real filesystem path ``~/.hermes`` (IYARI home) when rebranding
