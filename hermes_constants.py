...
def set_hermes_home_override(path: str | Path | None) -> Token:
    """Set a context-local IYARI home override and return its reset token.
...
def reset_hermes_home_override(token: Token) -> None:
    """Restore the previous context-local IYARI home override."""
    _HERMES_HOME_OVERRIDE.reset(token)


def get_hermes_home_override() -> str | None:
    """Return the active context-local IYARI home override, if any."""
...
def _get_platform_default_hermes_home() -> Path:
    """Return the platform-native default IYARI home path."""
...
def get_hermes_home() -> Path:
    """Return the IYARI home directory (default: platform-native path).
    ...
    ``hermes_cli/kanban_db.py``).  See https://github.com/digital-services-llc/iyari/issues/18594.
    """
...
def hermes_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a IYARI home/profile directory.
...
def get_process_hermes_home() -> Path:
    """Return the IYARI home for the running process, ignoring task overrides.
...
def get_default_hermes_root() -> Path:
    """Return the root IYARI directory for profile-level operations.

    In standard deployments this is the platform-native IYARI home
...
def get_hermes_dir(
    ...
    *,
    ...
    """Resolve a IYARI subdirectory with backward compatibility.
    ...
    Args:
        ...
        home: Optional explicit IYARI home. Profile-aware callers that manage
...
def iter_hermes_node_dirs(home: Path | None = None) -> list[Path]:
    """Return IYARI-managed Node.js directories in preferred lookup order.
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    IYARI-managed Node trees live under ``$HERMES_HOME/node`` (or a profile's
...
def hermes_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any IYARI-managed node/npm/npx shim exists on disk."""
    names = set()
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ IYARI-managed Node.js is in use by a running app; deferring its "
...
def bootstrap_hermes_managed_node() -> str | None:
    """Install a IYARI-managed Node tree and return its npm path.
    ...
    IYARI never modifies a toolchain it does not own, so instead it provisions
...
def heal_hermes_managed_node() -> bool:
    """Redownload IYARI-managed Node when the tree exists but is broken.
...
def find_hermes_node_executable(command: str) -> str | None:
    """Return a IYARI-managed Node/npm executable path, healing broken trees.
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for IYARI-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy IYARI-managed installs.

    This is for IYARI-owned subprocesses that should not be broken by a bad,
...
def with_hermes_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with IYARI-managed Node directories prepended to PATH."""
...
def secure_parent_dir(path: Path) -> None:
    ...
    See https://github.com/digital-services-llc/iyari/issues/25821 and
    https://github.com/digital-services-llc/iyari/pull/93050.
    """
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding IYARI profile HOME.

    ``HERMES_HOME`` scopes IYARI state. ``HOME`` is reserved for the OS/user
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply IYARI's subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when IYARI itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: digital-services-llc/iyari#47111
    """
...
# Top-level packages/modules that ship as part of IYARI itself. An ImportError
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with IYARI.
