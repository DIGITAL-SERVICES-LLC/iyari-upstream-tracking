...
- OpenViking server running and reachable from IYARI

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
IYARI can identify older servers that expose the legacy status-only health
...
connection values into IYARI, or create a minimal `ovcli.conf` when one does
not exist.
...
OpenViking's server config is separate from IYARI:
...
IYARI-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `hermes` | IYARI peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, IYARI lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
IYARI sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
current user). IYARI caches a confirmed user only for the active connection.
If the probe fails, IYARI uses the configured user, or `default`, for that
...
IYARI built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| IYARI action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because IYARI
