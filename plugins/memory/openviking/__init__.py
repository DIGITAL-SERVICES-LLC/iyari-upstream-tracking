...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — IYARI peer ID in OpenViking (default: hermes)
...
_AGENT_PROMPT_LABEL = "IYARI peer ID in OpenViking"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip IYARI slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach IYARI to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; IYARI refused the connection."
        ) from exc
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. IYARI did not start "
    ...
    try:
        ...
        # The IYARI Desktop backend can include the IYARI venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the IYARI venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the IYARI venv,
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; IYARI will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new IYARI session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in IYARI only", "write values only to IYARI .env"),
    ...
    _print_openviking_ready("Connection saved to IYARI .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "IYARI peer ID in OpenViking, sent as the actor peer and "
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "IYARI will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; IYARI will retry on a "
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of IYARI's full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert IYARI canonical messages into OpenViking batch payloads."""
