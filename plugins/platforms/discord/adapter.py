...
def _needs_server_members_intent(
    ...
    """Return True when IYARI must request Discord's Server Members intent.
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the IYARI gateway
    ...
    def _classify_connect_exception(self, error: Exception) -> tuple:
        ...
        if _is("PrivilegedIntentsRequired"):
            # Name the exact intents IYARI requested (#79430): Message
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage IYARI response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs IYARI work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit IYARI/gateway outage notices."""
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields IYARI manages."""
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit IYARI pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your IYARI session")
        ...
        @tree.command(name="status", description="Show IYARI session status")
        ...
        @tree.command(name="stop", description="Stop the running IYARI agent")
        ...
        @tree.command(name="update", description="Update IYARI to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the IYARI gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a IYARI session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to IYARI in the thread",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new IYARI session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by IYARI: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        IYARI has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "IYARI"
        if len(content) > 80:
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by IYARI: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="IYARI semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 IYARI handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want IYARI to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **IYARI needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ IYARI could not create a Discord thread for "
...
def register(ctx) -> None:
    """Plugin entry point — called by the IYARI plugin system."""
