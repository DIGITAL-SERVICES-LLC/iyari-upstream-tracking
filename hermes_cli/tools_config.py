"""
Unified tool configuration for IYARI.
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "IYARI routes X searches through xAI's built-in x_search "
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the IYARI telemetry policy applied.
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports IYARI's runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # misses the uv IYARI installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    # An installed driver that fails IYARI's runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # IYARI's own minimum requirement IS the confirmation that an upgrade is
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but IYARI cannot use its current "
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / IYARI process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the IYARI-managed ``$HERMES_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary IYARI installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # — it resolves lazily via npx (or a global/IYARI-managed install)
        ...
        # (PATH -> Homebrew/IYARI-managed node -> npx) rather than a bare
        # shutil.which — IYARI-managed-Node-only setups resolve agent-browser
        ...
        # IYARI-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — IYARI-managed-Node-only setups resolve npx only through the
        ...
        _print_info("    Restart IYARI for tracing to take effect.")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ IYARI Tool Configuration", Colors.CYAN, Colors.BOLD))
