...
Diagnoses issues with IYARI setup.
"""
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all IYARI containers"
...
def _hermes_database_paths(hermes_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for IYARI-managed SQLite databases."""
...
def _report_database_journal_modes(
    ...
    try:
        ...
        check_warn(f"Could not list IYARI databases: {exc}")
        return
    exposed = []
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed IYARI Relay plugin."""
...
def run_doctor(args):
    ...
    print(color("│                 🩺 IYARI Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: IYARI already refuses to enable WAL on fresh DBs.
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses IYARI's own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give IYARI a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# IYARI Persona\n\n"
                "<!-- Edit this file to customize how IYARI communicates. -->\n\n"
                "You are IYARI, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    if state_db_path.exists():
        ...
        # with a runaway WAL was previously invisible to every IYARI
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a IYARI one.
                        check_info(
