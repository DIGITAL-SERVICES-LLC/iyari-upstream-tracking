"""
Configuration management for IYARI.
...
#   the next ``subprocess.run([...])`` IYARI makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. IYARI itself starts
...
#   ``HERMES_ENV`` — IYARI runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # IYARI runtime location — never via dashboard env writer.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or IYARI runtime location "
...
def is_managed() -> bool:
    """Check if IYARI is running in package-manager-managed mode.
...
_NIX_UPDATE_MSG = (
    "Update IYARI through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how IYARI was installed: 'apt', 'docker', 'nix', 'nixos',
...
#     — that script installs a *new* host-side IYARI, it doesn't update
...
IYARI runs as a published image (nousresearch/hermes-agent), not a
...
def format_managed_message(action: str = "modify this IYARI installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this IYARI installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall IYARI."
    )
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running IYARI commonly set these to map the in-container
...
def _is_container() -> bool:
    ...
    When IYARI runs in a container with volume-mounted config files, forcing
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but IYARI's own config writer has historically emitted it
    ...
    # IYARI (``_save_discovered_models_to_config``), not hand-curated.
    # Older IYARI versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older IYARI versions) may write
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — IYARI won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the IYARI host rewrites it to the host or container
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset IYARI writes itself."""
...
def reload_env() -> int:
    ...
    the .env file (but only vars known to IYARI — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known IYARI vars that are no longer in .env
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    ...
    Used for IYARI-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ IYARI Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
