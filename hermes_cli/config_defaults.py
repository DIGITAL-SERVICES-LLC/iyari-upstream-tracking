"""Default configuration data for IYARI.
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["hermes-cli"],
    # SQLite journal mode used by every IYARI database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running IYARI server processes.
    ...
    "agent": {
        ...
        # the IYARI-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps IYARI
        ...
        # app, ACP) in a code workspace, IYARI adds a coding operating brief
    ...
    "terminal": {
        ...
        # skipped. When empty, IYARI auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), IYARI sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when IYARI
        ...
        # bundled IYARI image, which drops to the `hermes` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, IYARI sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and IYARI should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # AGENTS.md, CLAUDE.md, .hermes.md, or .cursorrules before IYARI applies
    ...
    # single read_file page exceeds these limits, IYARI truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so IYARI's summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); hermes =
                                      # IYARI's compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. IYARI's local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before IYARI does.
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; IYARI appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
    ...
    # "Hey IYARI" hands-free wake word. Always-on, on-device hotword
    ...
    # response. If not, IYARI feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before IYARI auto-pauses the goal and
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *IYARI*" header.
    ...
    # replace IYARI's built-in platform hint for a single messaging platform
    ...
    "security": {
        ...
        "acked_advisories": [],
        # Allow IYARI to lazy-install opt-in backend packages from PyPI
    ...
    #
    # Core IYARI tools (terminal, read_file, write_file, patch,
    ...
    # #8731).
    #
    # Provider keys accept the IYARI provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # behavior: files must live under the IYARI cache, under
        ...
        # uploaded as native gateway attachments. Files inside the IYARI
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # Bitwarden due to NETWORK/TIMEOUT, IYARI may use this encrypted
    ...
    "computer_use": {
        ...
        # by default upstream. IYARI disables it for our users unless they
        # explicitly opt in here. When false (default), IYARI sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; IYARI also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each IYARI computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". IYARI passes the
    ...
    # IYARI works exactly as before with `enabled: false`.
    "proxy": {
    ...
    # IYARI Desktop (Electron app) launch options. These only affect
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials IYARI mints short-lived tokens from. Falls back to "
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the IYARI auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey IYARI's wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use IYARI, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote IYARI API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote IYARI API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote IYARI API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
