"""On-demand supply-chain audit for IYARI installs.

Scans three surfaces a IYARI user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The IYARI venv (every PyPI dist via ``importlib.metadata``).
