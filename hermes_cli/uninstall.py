...
def remove_path_from_shell_configs():
    """Remove IYARI PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# IYARI" comment and following line
                if '# IYARI' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    the FHS fix).  Only symlinks that resolve into this IYARI home's ``node``
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify IYARI-owned User-PATH entries."""
...
def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip IYARI-owned entries from User-scope PATH in the registry.
...
def run_gui_uninstall(args):
    ...
    print(color("│         ⚕ IYARI Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No IYARI Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The IYARI agent stays installed.", Colors.CYAN))
    print()
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The IYARI agent at {hermes_home / 'hermes-agent'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The IYARI agent is still installed. Run 'hermes' to use the CLI,")
...
def run_uninstall(args):
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ IYARI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL IYARI data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the IYARI code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, hermes_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • IYARI PATH entries from shell configs / Windows User PATH")
    print("  • IYARI wrapper scripts and IYARI-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • IYARI config/data: {hermes_home}")
        ...
        print(f"  • Keep IYARI config/data: {hermes_home}")
    print()
    ...
    *,
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No IYARI-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No IYARI-set User env vars to remove")
    ...
    #     (only when they still point into this IYARI home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing IYARI-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No IYARI-managed node/npm/npx symlinks found")
    ...
    print()
    print("Thank you for using IYARI! ⚕")
    print()
