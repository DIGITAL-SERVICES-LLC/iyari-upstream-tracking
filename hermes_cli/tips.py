...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or iyari -c.",
    ...
    "Ctrl+Z suspends IYARI to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "iyari -c resumes your most recent CLI session. iyari -c \"project name\" resumes by title.",
    "iyari -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "iyari -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "iyari chat -t web,terminal enables only specific toolsets for a focused session.",
    "iyari chat -s github-pr-workflow preloads a skill at launch.",
    "iyari chat -q \"query\" runs a single non-interactive query and exits.",
    "iyari chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "iyari chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "iyari --yolo bypasses all dangerous command approval prompts for the entire session.",
    "iyari chat --source telegram tags the session for filtering in iyari sessions list.",
    "iyari -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "iyari doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "iyari dump outputs a compact setup summary — great for bug reports.",
    "iyari config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "iyari config edit opens config.yaml in your default editor.",
    "iyari config check scans for missing or stale configuration options.",
    "iyari sessions browse opens an interactive session picker with search.",
    "iyari sessions stats shows session counts by platform and database size.",
    "iyari sessions prune --older-than 30 cleans up old sessions.",
    "iyari skills search react --source skills-sh searches the skills.sh public directory.",
    "iyari skills check scans installed hub skills for upstream updates.",
    "iyari skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "iyari skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "iyari mcp add github --command npx adds MCP servers from the command line.",
    "iyari mcp serve runs IYARI itself as an MCP server for other agents.",
    "iyari auth add lets you add multiple API keys for credential pool rotation.",
    "iyari completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "iyari logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "iyari backup creates a zip backup of your entire IYARI home directory.",
    "iyari profile create coder creates an isolated profile that becomes its own command.",
    "iyari profile create work --clone copies your current config and keys to a new profile.",
    "iyari update syncs new bundled skills to ALL profiles automatically.",
    "iyari gateway install sets up IYARI as a system service (systemd/launchd).",
    "iyari memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "iyari webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: iyari tools disables unused tools, iyari skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "iyari models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as iyari -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call IYARI tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'iyari profile create coder' creates the 'coder' command.",
    "iyari profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, IYARI prints a resume command with session ID and stats.",
    "iyari sessions export backup.jsonl exports all sessions for backup or analysis.",
    "iyari -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "iyari skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: iyari cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "IYARI runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "iyari gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to IYARI via a local macOS server.",
    ...
    "IYARI loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "iyari mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "iyari mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "iyari mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — iyari tools can toggle them per platform.",
    ...
    "iyari chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "iyari plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "iyari acp runs IYARI as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "iyari auth add nous or iyari auth add openai-codex sets up OAuth-based providers.",
    ...
    "iyari status --deep runs deeper diagnostic checks across all components.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make IYARI your own.",
    ...
    "Before a gateway session reset, IYARI auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope IYARI state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'iyari profile create ls' would be rejected.",
    "iyari profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "iyari claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via iyari tools.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — IYARI auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of IYARI config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while IYARI is working.',
    ...
    # --- Curator ---
    'iyari curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "iyari curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'iyari curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'iyari auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'iyari dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'iyari dashboard embeds the full IYARI TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'iyari -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'iyari chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'iyari chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'iyari chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "iyari chat --source tool tags programmatic chats so they don't clutter iyari sessions list.",
    'iyari dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'iyari sessions rename <ID> "new title" renames any past session; iyari sessions delete <ID> removes one.',
    'iyari import restores a full IYARI backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'iyari fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'iyari pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'iyari setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'iyari status --deep runs the full health sweep across every component; plain iyari status is the quick view.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes IYARI block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from iyari sessions list by default — set --source explicitly to see them.',
