...
``<root>`` is the **shared IYARI root** (the parent of any active
...
the profile's IYARI home but are otherwise isolated: a worker spawned
...
def kanban_home() -> Path:
    """Return the shared IYARI root that anchors the kanban board.
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources IYARI treats as user-controlled-but-trusted on the
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold IYARI's own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside IYARI-managed storage. A board ``default_workdir`` pointing at a
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the IYARI checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, IYARI treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, IYARI creates/reuses that linked worktree. With
      no ``workspace_path``, IYARI anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, IYARI uses
      ``wt/<task-id>``.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a IYARI
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real IYARI profile.
...
def review_dispatch_enabled() -> bool:
    ...
    The default is true because IYARI ships the ``sdlc-review`` skill and the
...
# Assumed per-worker memory footprint for the derived default cap. IYARI
...
def _dispatch_once_locked(
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real IYARI profile.
        # `_default_spawn` invokes ``hermes -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a IYARI
    ...
    # Auto-dispatch is enabled by default because IYARI bundles the
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound IYARI CLI invocation."""
...
def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved IYARI shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved IYARI executable path.
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       IYARI is launched from a venv and the ``hermes`` shim is not on
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install IYARI or activate its venv before running the kanban dispatcher."
        )
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default IYARI root exists
