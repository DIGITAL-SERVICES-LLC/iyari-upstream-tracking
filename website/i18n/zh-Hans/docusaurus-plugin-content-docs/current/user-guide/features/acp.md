...
sidebar_position: 11
title: "ACP 宿主集成"
description: "在兼容 ACP 的编辑器和协作平台中使用 IYARI"
---

# ACP 宿主集成

IYARI 可作为 ACP 服务器运行，让兼容 ACP 的编辑器通过 stdio 与 IYARI 通信并渲染：
...
当你希望 IYARI 表现得像编辑器原生的编码 agent，而非独立 CLI 或消息机器人时，ACP 是合适的选择。

## IYARI 在 ACP 模式下暴露的内容

IYARI 使用专为编辑器工作流设计的精选 `hermes-acp` 工具集运行，包括：
...
IYARI 将日志输出到 stderr，以保留 stdout 用于 ACP JSON-RPC 流量。
...
- 将 `npm install -g agent-browser @askjo/camofox-browser` 安装到该前缀（无需 sudo — `npm` 的 `--prefix` 指向用户可写的 IYARI 管理 Node）
...
这是一种传输层集成，不是第二个 IYARI 安装。由 `buzz-acp` 启动的子进程使用该主机上
与 `hermes` 相同的配置、凭据、记忆、技能和状态。

（这与 [Buzz Desktop 的托管运行时](#buzz-desktop)不同——后者在本地将 IYARI 作为
...
- 为 IYARI 铸造专用的 Nostr 密钥对（`buzz-admin generate-key`）并将其注册为
...
若要持久化部署到服务器，请以拥有目标 IYARI home 的同一操作系统用户身份，
在服务管理器下运行 `buzz-acp`。安装、密钥生成、频道发现和各项 agent 选项见
[buzz-acp README](https://github.com/block/buzz/tree/main/crates/buzz-acp)。

桥接会发现 IYARI 身份所属的每个 Buzz 频道，并在其被加入新频道时自动订阅。
因此 Buzz 频道成员资格就是访问边界；IYARI 自身配置中无需单独的频道列表。

若要在所有者的 Buzz Desktop 中展示 IYARI 的 ACP 活动，添加：
...
2. 从内置 agent 列表中选择 **IYARI**。
3. 连接并开始聊天。

如需手动定义 IYARI，通过 VS Code 设置在 `acp.agents` 下添加：
...
在 Zed 设置中将 IYARI 配置为自定义 agent 服务器：
...
3. 启动新的 IYARI 外部 agent 线程。

前提条件：

- 先通过 `hermes model` 配置 IYARI provider 凭据，或在 `~/.hermes/.env` / `~/.hermes/config.yaml` 中设置。
...
[Buzz](https://github.com/block/buzz) 将 IYARI 作为预设运行时提供。
按常规方式安装 IYARI 后，Buzz 会自动发现它 —— 打开 **Settings → Runtimes**，
IYARI 就会出现在你的运行时列表中。
...
当运行时为 IYARI 时，请保持该设置。

这条路径上有两种行为叠加。`hermes-acp` 工具集包含 `terminal` 和 `execute_code`，
而 Buzz 的 ACP 桥接层会自行以 `allow_once` 回应 IYARI 的权限请求，不会转交给你确认。
因此 Buzz 中的 IYARI agent 会在不提示的情况下在宿主机上执行 shell 命令。
...
- `approvals.mode: manual` 确实会让 IYARI 发出权限请求，但 Buzz 仍会自动批准，
  命令照样执行。
...
Provider 解析使用 IYARI 的正常运行时解析器，因此 ACP 继承当前配置的 provider 和凭据。IYARI 还为首次运行的 ACP 客户端提供终端认证方法（`--setup`）；这将打开 IYARI 的交互式模型/provider 设置。
...
底层 `AIAgent` 仍使用 IYARI 的正常持久化/日志路径，但 ACP 的 `list/load/resume/fork` 仅限于当前运行的 ACP 服务器进程。

## 工作目录行为

ACP 会话将编辑器的 cwd 绑定到 IYARI 任务 ID，使文件和终端工具相对于编辑器工作区运行，而非服务器进程的 cwd。
...
| `allow_always` | 始终允许 | 所有未来会话 | 是（写入 IYARI 永久允许列表） |
| `deny` | 拒绝 | 本次工具调用 | 否 |
...
ACP 桥接将这些选项映射到 IYARI 的内部审批语义——`allow_always` 与 CLI 相同地写入永久允许列表条目，而 `allow_session` 仅影响当前 ACP 会话的进程内审批缓存。
...
ACP 模式使用 IYARI 现有的 provider 设置。通过以下方式配置凭据：
