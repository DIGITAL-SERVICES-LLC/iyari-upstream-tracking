---
sidebar_position: 8
title: "Use Voice Mode with IYARI"
description: "A practical guide to setting up and using IYARI voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with IYARI
...
- you want IYARI sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in IYARI.
...
## Step 1: make sure normal IYARI works first
...
IYARI supports both local and cloud speech stacks.
...
If you choose NeuTTS in the setup wizard, IYARI checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. IYARI transcribes and responds
...
- asking IYARI to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full IYARI loop.

## Tuning CLI behavior

### Silence threshold

If IYARI starts/stops too aggressively, tune:
...
IYARI stays a normal chat bot, but can speak replies.
...
- you want IYARI to function like a portable research or ops assistant
...
IYARI joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- IYARI detects speech boundaries
- transcripts are posted in the associated text channel
- IYARI responds in text and audio
