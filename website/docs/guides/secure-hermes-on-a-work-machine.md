---
sidebar_position: 26
title: "Running IYARI on a Personal or Work Machine"
description: "A security-posture walkthrough for running IYARI on the machine you live on — what the defaults protect, how to tighten further, and how to undo mistakes"
---

# Running IYARI on a Personal or Work Machine
...
**Your data goes only where you point it.** API calls go **only to the LLM provider you configure**. IYARI does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.hermes/`. See the [FAQ](/reference/faq#is-my-data-sent-anywhere).
...
:::caution
Don't add this to `~/.hermes/.env` casually. If you set it to a project directory only, the agent cannot write to `~/.hermes/cron/jobs.json`, profile skills, or other IYARI state outside that prefix. Include your IYARI home as a second root, as above.
:::
...
:::tip
Preview with `/rollback diff <N>` before restoring, and combine checkpoints with git worktrees for maximum safety — each IYARI session in its own worktree, with checkpoints as an extra layer.
:::
