---
sidebar_position: 16
title: "Manage IYARI Cloud with MCP"
description: "Connect IYARI to the Nous Portal MCP server so your local agent can list, start, stop, and manage your IYARI Cloud instances conversationally"
---

# Manage IYARI Cloud with MCP

[Hermes Cloud](https://portal.nousresearch.com/cloud) runs hosted Hermes Agent instances for you. Normally you manage them from the `/agents` page in the [Nous Portal](/integrations/nous-portal). This guide connects your **local** IYARI to the Portal's MCP server so you can manage those cloud instances by just asking — "list my cloud agents", "restart the stopped one", "what's it costing me" — without leaving your terminal.

It's a standard [MCP](/user-guide/features/mcp) server hosted by Nous Research, gated by the same OAuth login you already use for the Portal. Once connected, IYARI gets two tools it can call on your behalf.

## What you can do with it

Once connected, the model can call these on your IYARI Cloud org:
...
`--auth oauth` tells IYARI this is an OAuth-protected HTTP server. On first connect IYARI:
...
```text
List my IYARI Cloud agents and their current status.
```
...
```text
Create a new IYARI Cloud instance named scratch, then tell me when it's ready.
```

IYARI reports what each tool returned — the instance list, the new status, the created instance's details — so you can confirm the action landed.
...
Run `/reload-mcp` after changing the config. See [Use MCP with IYARI](/guides/use-mcp-with-hermes) for the full filtering model (`include`/`exclude`, `prompts`, `resources`).

## Troubleshooting

### The browser shows an org picker and I'm not sure which to choose

You belong to multiple Portal organizations. Pick the org whose IYARI Cloud instances you want to manage from this connection. If you're unsure, it's the org that owns the instances you see on the Portal `/agents` page. You can re-choose later by removing and re-adding the server.
...
The OAuth browser callback runs on the machine where IYARI is running. On a remote host, forward the loopback port over SSH — the same pattern as any other OAuth login. See [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh).

## See also

- **[Nous Portal](/integrations/nous-portal)** — the subscription, models, and Tool Gateway behind the same login
- **[Use MCP with IYARI](/guides/use-mcp-with-hermes)** — connecting and filtering MCP servers in general
- **[MCP feature overview](/user-guide/features/mcp)** — what MCP is and how IYARI uses it
