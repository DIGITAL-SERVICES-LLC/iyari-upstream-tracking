...
Copy-paste blueprints for common automation patterns. Each blueprint uses IYARI's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
hermes cron create "0 2 * * *" \
  "You are a project manager triaging the digital-services-llc/iyari GitHub repo.

1. Run: gh issue list --repo digital-services-llc/iyari --state open --json number,title,labels,author,createdAt --limit 30
...
hermes cron create "0 9 * * 1" \
  "Scan the digital-services-llc/iyari repo for documentation drift.

1. Run: gh pr list --repo digital-services-llc/iyari --state merged --json number,title,files,mergedAt --limit 30
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "IYARI-Monitor/1.0"})
...
```bash
hermes cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to IYARI development." \
