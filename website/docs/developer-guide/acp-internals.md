...
The ACP adapter wraps IYARI's synchronous `AIAgent` in an async JSON-RPC stdio server.
...
- `allow_once` -> IYARI `once`
- `allow_always` -> IYARI `always`
- reject options -> IYARI `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps IYARI tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses IYARI's runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured IYARI provider/credentials. It also always advertises a terminal setup auth method (`hermes-setup`, args `--setup`) so first-run ACP clients can open IYARI's interactive model/provider configuration before starting a normal ACP session.
