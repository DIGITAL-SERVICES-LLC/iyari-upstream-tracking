...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept IYARI already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — IYARI provider id (e.g. "openrouter")
...
using IYARI built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different IYARI provider
        # (must be one IYARI already knows about — same names as
        ...
        # plugin sends — IYARI does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through IYARI's usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing IYARI subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/digital-services-llc/iyari/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/digital-services-llc/iyari/blob/main/tests/agent/test_plugin_llm.py)
