...
This guide covers adding a new messaging platform to the IYARI gateway. A platform adapter connects IYARI to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
The plugin system lets you add a platform adapter without modifying any core IYARI code. Your plugin is a directory with two files:
...
With `provides_tools` declared, IYARI imports only `tools.py` during plugin
...
def register(ctx):
    ctx.register_platform(
        ...
        # May be a regular function or async def. IYARI awaits any awaitable
...
:::note
This checklist is for adding a platform directly to the IYARI core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
