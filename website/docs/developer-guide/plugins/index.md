...
title: "Build a IYARI Plugin"
description: "Step-by-step guide to building a complete IYARI plugin with tools, hooks, data files, and skills"
---

# Build a IYARI Plugin

This guide walks through building a complete IYARI plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
IYARI has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `digital-services-llc/iyari`. Users install them into `~/.hermes/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it on [IYARI on GitHub](https://github.com/digital-services-llc/iyari). See [CONTRIBUTING.md](https://github.com/digital-services-llc/iyari/blob/main/CONTRIBUTING.md) for the policy.
:::

## Portable Agent Plugins v1 packages

IYARI can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components IYARI already owns. It does not replace native `plugin.yaml` plus
...
IYARI validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by IYARI.
...
Portable `streamable-http` entries are routed through IYARI's existing native
...
same full-trust posture as other installed IYARI plugins.
...
records it as Published. IYARI keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. IYARI does not expose a
...
continue to work after a normal IYARI upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  IYARI inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older IYARI releases can therefore load a plugin whose manifest contains
...
IYARI enforces this contract with frozen external-plugin fixtures discovered
...
registry used by IYARI itself. It reports invalid hook names, callbacks that do
...
This tells IYARI: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
IYARI reads these from installed metadata without importing your code, so
...
this IYARI understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — IYARI validates them, and `hermes plugins install` / `hermes plugins doctor` surface missing ones with a `pip install` hint, but IYARI **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into IYARI's shared venv is a conflict and supply-chain
...
[#64165](https://github.com/digital-services-llc/iyari/issues/64165) and
[#15220](https://github.com/digital-services-llc/iyari/issues/15220). Plugin
...
4. **Accept `**kwargs`:** IYARI may pass additional context in the future.
...
- If this function crashes, the plugin is disabled but IYARI continues fine
...
Use plugin-relative config keys for user-visible behavior. IYARI resolves them
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing IYARI into installing arbitrary packages — only specs IYARI itself ships are eligible |
...
does not ship with IYARI core (user, project, or pip source),
...
loader, the plugin is disabled and IYARI continues. The gate exists
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), IYARI injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.
...
- **The system prompt is IYARI's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before IYARI sends them. |
...
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer IYARI still loads on an older one.
...
For running a full `hermes <subcommand>` (e.g. `hermes kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "hermes kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive IYARI from a hook.
...
IYARI has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.hermes/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
IYARI also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into IYARI without any Python plugin. Declare them in `~/.hermes/config.yaml`:
...
IYARI connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).
...
```python
# Wrong — will break if IYARI passes extra context
def handler(args):
    ...
