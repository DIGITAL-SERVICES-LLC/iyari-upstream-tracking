...
description: "How IYARI builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

IYARI deliberately separates:
...
You are IYARI, an AI assistant created by Digital Services LLC.
...
The platform hint (Layer 10 above) is the per-surface guidance IYARI
...
|----------|-------|-------------|-------|
| 1 | `.hermes.md`, `HERMES.md` | CWD up to git root | IYARI-native project config |
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, IYARI concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs IYARI already loads, rather than editing Python templates in place.
...
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking IYARI.
...
- if you want to change how IYARI assembles prompts for everyone, change Python and treat it as a code contribution
