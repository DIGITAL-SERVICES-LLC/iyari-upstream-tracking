...
description: "How to build a memory provider plugin for IYARI"
---

# Building a Memory Provider Plugin

Memory provider plugins give IYARI persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.
...
IYARI discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with IYARI. Closed to new providers — see [CONTRIBUTING](https://github.com/digital-services-llc/iyari/blob/main/CONTRIBUTING.md). |
...
IYARI, in `$HERMES_HOME/plugins/<name>/` when installed by a user, or in
...
two files IYARI reads from disk rather than importing — `config_schema.py`
...
The gate binds to every compaction authority, not just the IYARI
...
required checkpoint cannot be guaranteed there. The checkpoint-aware IYARI
...
the `messages` parameter; IYARI will continue calling them with the legacy
signature.
