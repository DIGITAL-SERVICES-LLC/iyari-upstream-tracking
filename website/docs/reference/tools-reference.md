...
:::tip MCP Tools
In addition to built-in tools, IYARI can load tools dynamically from MCP servers. MCP tools appear with the prefix `mcp__<server>__` (e.g., `mcp__github__create_issue` for the `github` MCP server). See [MCP Integration](/user-guide/features/mcp) for configuration.
:::
...
| `kanban_attach_url` | Attach a file to a task by URL — IYARI downloads it server-side and stores it as a real attachment (capped at 25 MB). Only http/https URLs. | `HERMES_KANBAN_TASK` or `kanban` toolset |
...
Enabled for sessions whose source is the IYARI desktop app, on any backend it
is connected to (local, SSH, URL, or IYARI Cloud). Absent from CLI, TUI,
messaging, and cron sessions.

| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `read_terminal` | Read what's currently shown in the in-app terminal pane of the IYARI desktop GUI (the embedded shell beside this chat). | — |
| `close_terminal` | Close the read-only terminal tab for a background process in the IYARI desktop GUI. Does NOT kill the process — only drops the tab/view; use process(action='kill') to stop it. | — |
| `open_preview` | Open a web URL, localhost dev-server URL, or file path in the preview pane beside the chat in the IYARI desktop app. | — |
...
| `read_preview` | Read what's currently shown in the preview pane of the IYARI desktop GUI — the in-app Browser's page text (URL + title + rendered text, pageable with `start`/`count`), or a file/artifact tab's identity. | — |
...
| `read_window_below` | Identify the OS window directly underneath the IYARI desktop window — app name, title, bounds (metadata only, never pixels). On macOS, other apps' titles appear only when Screen Recording is already granted; the tool never prompts for it. | — |
| `focus_pane` | Reveal and focus a pane in the IYARI desktop app (chat, files, terminal, review, sessions). | — |
...
| `tour` | Give a live guided tour: dim the screen, highlight an element, and attach a narrated popover (driver.js). Works on the IYARI app's own UI and on any page open in the preview pane; `targets` discovers what's on screen, `show` narrates step-by-step, `start` hands the user Next/Prev controls. | — |
