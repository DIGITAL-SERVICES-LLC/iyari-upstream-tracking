...
description: "Authoritative reference for IYARI terminal commands and command families"
---
...
| `--profile <name>`, `-p <name>` | Select which IYARI profile to use for this invocation. Overrides the sticky default set by `hermes profile use`. |
...
| `hermes peer` | Register peer IYARI gateways on other machines and DM their agents' canonical Bot Chats (`hermes peer dm <peer>[/<agent>] "…"`). The transport behind cross-machine bot-to-bot messaging. |
...
| `hermes backup` | Back up IYARI home directory to a zip file. |
...
| `hermes import` | Restore a IYARI backup from a zip file. |
| `hermes logs` | View, tail, and filter agent/gateway/error log files. |
| `hermes config` | Show, edit, migrate, and query configuration files. |
| `hermes skin` | List, switch, and tweak display skins. |
| `hermes console` | Open the safe IYARI command console. |
...
| `hermes acp` | Run IYARI as an ACP server for editor integration. |
| `hermes mcp` | Manage MCP server configurations and run IYARI as an MCP server. |
| `hermes plugins` | Manage IYARI plugins (install, enable, disable, remove). |
...
| `hermes serve` | Start the IYARI backend server (headless; powers the desktop app and remote backends). |
| `hermes desktop` (alias `gui`) | Build and launch the native Electron desktop app. |
| `hermes profile` | Manage profiles — multiple isolated IYARI instances. |
...
| `hermes uninstall` | Remove IYARI from the system. |
...
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, shell hooks, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from IYARI itself. |
...
hermes chat --safe-mode -q "Is this bug mine or IYARI's?"
```
...
Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active IYARI chat session.
...
**`hermes model`** (run from your terminal, outside any IYARI session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active IYARI chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your IYARI session first (`Ctrl+C` or `/quit`), then run `hermes model` from your terminal prompt.
:::
...
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [IYARI Relay](/user-guide/messaging/relay). |
...
| `--name NAME` | `IYARI` | Bot display name in Slack. |
...
Bot-to-bot DMs across machines. Register another IYARI gateway (any machine
...
> Not to be confused with `hermes claw migrate` (one-shot import of OpenClaw configuration into IYARI) — `hermes migrate` is the top-level config-rewrite command.
...
On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the IYARI venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.hermes/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
| `--skip-venv` | off | Skip scanning the IYARI Python venv. |
...
hermes auth spotify                                      # Authenticate IYARI with Spotify via PKCE
```
...
Outputs a compact, plain-text summary of your entire IYARI setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | IYARI version, release date, git commit hash |
...
The report includes system info (OS, Python version, IYARI version), recent agent, gateway, GUI/dashboard, and desktop logs (512 KB limit per file), and redacted API key status. By default, uploads are redacted so secrets are not included.
...
Create a zip archive of your IYARI configuration, skills, sessions, and data. The backup excludes the hermes-agent codebase itself, and it does not nest earlier backup artifacts (`backups/`, `state-snapshots/`) — each of those already contains its own copy of `state.db`.
...
The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when IYARI is running (WAL-mode safe).
...
Restore a previously created IYARI backup into your IYARI home directory. All files in the archive overwrite existing files in your IYARI home; `--force` only skips the confirmation prompt that fires when the target already has a IYARI installation.
...
View, tail, and filter IYARI log files. All logs are stored in `~/.hermes/logs/` (or `<profile>/logs/` for non-default profiles).
...
IYARI uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `hermes logs list` subcommand shows all log files including rotated ones.
...
- **Prompt tiers** — stable / context / volatile, matching how IYARI layers
...
- `--source well-known` lets you point IYARI at a site exposing `/.well-known/skills/index.json`.
...
`hermes moa configure` reuses IYARI's provider → model picker for each reference model and the aggregator. A preset is an execution-mode configuration, not a primary model or provider.
...
Starts IYARI as an ACP (Agent Client Protocol) stdio server for editor integration.
...
Manage MCP (Model Context Protocol) server configurations and run IYARI as an MCP server.
...
| `serve [-v\|--verbose]` | Run IYARI as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with IYARI](../guides/use-mcp-with-hermes.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server).
...
See [Plugins](../user-guide/features/plugins.md) and [Build a IYARI Plugin](../developer-guide/plugins/index.md).
...
If cua-driver is already present, IYARI checks its version and runtime
...
installer. IYARI never replaces a custom binary selected through
...
The built-in `computer_use` toolset is the recommended IYARI integration.
Registering raw Cua MCP tools is an alternative when you need Cua's low-level
tool vocabulary. `cua-driver skills install` detects IYARI and links Cua's
skill pack into the IYARI skills directory automatically.

Permission mode, capability-manifest approval, and the existing-profile grant
belong to runtime launch. In bounded mode IYARI passes Cua's canonical
...
right now without waiting for the next IYARI update.
...
[Petdex](https://github.com/crafter-station/petdex) is a public gallery of animated sprite pets for coding agents. Install one and IYARI shows it reacting to agent activity across the CLI, TUI, and desktop app.
...
Migrate your OpenClaw setup to IYARI. Reads from `~/.openclaw` (or a custom path) and writes to `~/.hermes`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing IYARI files on conflicts (default: refuse to apply when the plan has conflicts). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into IYARI equivalents or **archived** for manual review.
...
Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into IYARI. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.hermes/skills/`. Always previews before applying; API keys and credentials are never imported.
...
Manage profiles — multiple isolated IYARI instances, each with its own config, sessions, skills, and home directory.
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of IYARI commands, subcommands, and profile names.
...
`hermes update` pulls the configured update branch (default: `main`). If your checkout is on another branch, IYARI may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
- **Gateway restart.** After a successful update, IYARI attempts to restart all running gateway profiles automatically so they pick up the new code. Use `hermes gateway restart` when you want to restart a gateway without applying an update.
...
- **npm lockfile churn.** Before stashing or switching branches, IYARI makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `hermes update`.
...
- **Legacy `hermes.service` warning.** If IYARI detects a pre-rename `hermes.service` systemd unit (instead of the current `hermes-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| `hermes uninstall [--full] [--gui] [--dry-run] [--yes]` | Remove IYARI, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--dry-run` prints what would be removed without changing anything; `--yes` skips prompts. |
