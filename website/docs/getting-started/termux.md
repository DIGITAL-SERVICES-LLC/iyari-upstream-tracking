...
description: "Run IYARI directly on an Android phone with Termux"
---

# IYARI on Android with Termux
...
IYARI can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop IYARI from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed IYARI installs are marked with install method `apt`. IYARI therefore does not run its Git self-updater against package-owned files; use the package manager instead:
...
Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. IYARI runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

IYARI now ships a Termux-aware installer path:
