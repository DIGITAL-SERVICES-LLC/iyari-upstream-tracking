...
description: "Install and deploy IYARI with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
IYARI ships a Nix flake, a NixOS module, and a Home Manager module.
...
```bash
# Run the desktop app
nix run github:digital-services-llc/iyari#desktop

# Or install persistently
nix profile install github:digital-services-llc/iyari#desktop

# run the tui
nix run github:digital-services-llc/iyari -- setup
nix run github:digital-services-llc/iyari -- --tui
...
:::note
This module needs NixOS. IYARI is an agent for one person. If you want an agent for one person and not a system service, use the [Home Manager module](#home-manager-module). That module runs on NixOS and on each other system that Home Manager supports.
:::
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, IYARI defaults to OpenRouter.
:::
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). IYARI reads this file on every startup, so changes take effect with a `systemctl restart hermes-agent` — no container recreation needed.
...
IYARI reads files from two directories. Thus there are two options. Use the option for the directory that the file must go into.
...
`hermesHomeFiles` installs into **`HERMES_HOME`**. IYARI reads the identity file and the memory files of the agent from that directory. `SOUL.md` and `memories/` work only from there. A `SOUL.md` in `documents` makes a workspace file. IYARI does not load that file as the identity:
...
Set `auth = "oauth"` for servers using OAuth 2.1. IYARI implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
The first OAuth authorization requires a browser-based consent flow. In a headless deployment, IYARI prints the authorization URL to stdout/logs instead of opening a browser.
...
The flake also exports `homeManagerModules.default`. IYARI is an agent for one person. The credentials, the memory, the sessions and the cron jobs all belong to that person. Thus a user service is the correct shape on a personal machine. It runs on each distribution that Home Manager supports, and not only on NixOS.
...
`gateway.enable` runs the messaging gateway for Telegram, Discord, Slack and the other platforms. IYARI Desktop and the web dashboard connect to a *different* process, which is `hermes serve` or `hermes dashboard`. `backend.mode` runs that process with the gateway:
...
`serve` runs without a user interface. It gives the `/api/ws` and `/api/pty` sockets that IYARI Desktop connects to, and it does not build the web application. `dashboard` gives all of that, and also serves the browser admin panel. Both processes use one `HERMES_HOME` with the gateway. Thus the sessions, the skills, the memory and the cron jobs are the same for all of them. `backend.mode` works in the same way on the NixOS module, but not in container mode.
...
Plugins are symlinked into `$HERMES_HOME/plugins/` at activation time. IYARI discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
| `hermesHomeFiles` | `attrsOf (either str path)` | `{}` | Files that go into `HERMES_HOME`. `SOUL.md` and `memories/` must be here, or IYARI does not load them. |
...
This option runs the process that IYARI Desktop and the web dashboard connect to, with the gateway. You cannot use it with `container.enable`.
...
| `desktop.enable` | `bool` | `false` | Add the IYARI Desktop application, with a launcher entry on Linux |
| `desktop.package` | `package` | `package.hermesDesktop` | The desktop package. The default follows `package`, so the application and the services run one IYARI runtime. |
...
├── auth.json                        # OAuth credentials: seeded, then IYARI owns it
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | IYARI binary + all Nix deps |
