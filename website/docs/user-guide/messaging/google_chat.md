...
sidebar_position: 12
title: "Google Chat"
description: "Set up IYARI as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect IYARI to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your IYARI
...
download the file. Save it somewhere only IYARI can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — IYARI calls
...
- **App name**: whatever you want users to see ("IYARI" is reasonable).
...
`ADDED_TO_SPACE` event that IYARI uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure IYARI
...
Send "hola" in the test DM. The bot posts a "IYARI is thinking…" marker, then
...
platforms:
  google_chat:
    # Custom working-state marker text (default: "IYARI is thinking…").
...
Thread support: when a user replies inside a thread, IYARI detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate IYARI session.
...
3. Download the JSON. Move it onto the host that runs IYARI.
4. Register the client with IYARI (run under the profile you want it scoped to):
...
That writes the client secret into the active profile's IYARI home (e.g.
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, IYARI isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: IYARI will only attach the SA bearer
