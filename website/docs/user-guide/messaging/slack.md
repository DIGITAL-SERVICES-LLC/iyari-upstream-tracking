...
sidebar_position: 4
title: "Slack"
description: "Set up IYARI as a Slack bot using Socket Mode"
---

# Slack Setup

Connect IYARI to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your IYARI instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. IYARI uses the modern
...
The fastest path is to paste a manifest IYARI generates for you. It
...
### Option A: From a IYARI-generated manifest (recommended)
...
4. Enter an app name (e.g., "IYARI") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, IYARI can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a IYARI configuration issue.
:::
...
IYARI uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; IYARI now
...
Every IYARI command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
IYARI command with its description.

Under the hood: IYARI ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/digital-services-llc/iyari/blob/main/hermes_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing IYARI
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so IYARI can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. IYARI only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When IYARI adds new commands (e.g. after `hermes update`), regenerate
...
`/hermes btw run the tests` — IYARI routes it the same way as `/btw
...
Slack never delivers them to IYARI.

As a workaround, IYARI recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
IYARI treats it identically to the slash form and replies in the same
thread.
...
The bang form also works behind a mention (`@IYARI !stop`) and with
...
IYARI falls back to a text prompt, the prompt instructs you to reply
...
fails, IYARI retries via a second ephemeral API path — a slash reply is never
posted publicly to the channel as a fallback. (Commands typed as regular
messages — `!cmd` in threads, `@IYARI /cmd` — reply as normal visible
messages instead.)
...
Understanding how IYARI behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@IYARI what time is it?`). In channels, IYARI replies in a thread attached to that message. |
| **Threads** | If you @mention IYARI inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
platforms:
  slack:
    ...
    extra:
      ...
      # If Slack rejects the native stream, IYARI keeps one editable text
      ...
      # that message itself @mentions IYARI, and "all" accepts every
      # other bot. IYARI always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
| `platforms.slack.extra.allow_bots` | `"none"` | Controls messages from other Slack bots: `"none"` ignores them, `"mentions"` accepts a bot message only when **that message itself** @mentions IYARI, and `"all"` accepts all of them. Use `"mentions"` for the safest bot-to-bot collaboration mode. See [Accepting messages from other bots](#accepting-messages-from-other-bots-allow_bots). |
...
name in the thread. By default IYARI sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"IYARI is thinking…"`) —
...
`streaming.enabled` is on (transport `auto` or `draft`), IYARI uses native
streaming automatically wherever it's available:

- The stream starts on the first frame and appends only deltas (the API is
  append-only). The streamed message **is** the final message — IYARI seals
...
- If your Slack app doesn't have the AI features enabled (or lacks the
  `assistant:write` scope), the first failure is cached and IYARI falls back
...
- If the native stream can't start or update, IYARI falls back to a single
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to IYARI in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before IYARI will respond.
...
By default IYARI ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several IYARI instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- IYARI always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer IYARI agents are filtered consistently across workspaces.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where IYARI will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @IYARI`).
...
Delivery works even when the cron process isn't co-located with the gateway — IYARI falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
...
IYARI can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, IYARI also loads tokens from an **OAuth token file** at:
...
- When a message arrives, IYARI uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @IYARI` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @IYARI` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. IYARI now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @IYARI`)
...
- Audit who has access to your IYARI config directory
