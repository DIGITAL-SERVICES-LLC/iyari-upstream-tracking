...
description: "Set up IYARI as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect IYARI to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which IYARI strips automatically before processing.
...
The public tunnel URL uses HTTPS, but IYARI's local webhook listener uses plain HTTP. The tunnel terminates TLS and forwards HTTP to port `3978`; do not configure the local tunnel port as HTTPS.
...
The Teams SDK is optional; when Teams is enabled, the gateway lazy-installs it into IYARI's own venv on first start (do **not** use system `pip install` on Ubuntu 24.04 — that hits PEP 668 `externally-managed-environment`). To install manually into the IYARI venv:
...
For a permanent server, terminate TLS at a reverse proxy and forward requests to the plain HTTP IYARI listener, normally `http://127.0.0.1:3978`. Register the proxy's public HTTPS endpoint with Teams:
...
Make sure the public HTTPS endpoint is reachable from the internet and uses a valid TLS certificate. Teams rejects self-signed certificates. Keep the IYARI listener behind the proxy; port `3978` does not serve HTTPS itself.
...
| `requirements not met` / `Teams SDK missing` / `No adapter available for teams` | Restart gateway so lazy-install can run, or install into the **IYARI venv**: `~/.hermes/hermes-agent/venv/bin/pip install microsoft-teams-apps aiohttp`. System `pip` fails on Ubuntu 24.04 (PEP 668) and would not affect the service anyway |
...
| Logs show `"UNKNOWN / HTTP/1.0" 400` when Teams sends a message | The tunnel or reverse proxy is forwarding HTTPS to IYARI's plain HTTP listener. Terminate TLS at the proxy and forward HTTP to port `3978` |
