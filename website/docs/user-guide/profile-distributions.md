...
A **profile distribution** packages a complete IYARI agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## Two ways to share a profile

IYARI has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.
...
Before distributions, sharing a IYARI agent meant sending someone:
...
The tradeoff: recipients need git installed. On any machine running IYARI in 2026, that's already true.
...
:::caution
**IYARI does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `hermes profile install` or `hermes profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```
# Environment variables required by this IYARI distribution.
...
You built IYARI-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
**The default profile** (`~/.hermes`) is exported through an allow-list — only known IYARI artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
- [Using SOUL with IYARI](../guides/use-soul-with-hermes.md) — authoring personalities
