...
```
IYARI -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
3. Add `twozero_td` MCP server to IYARI config (if missing)
...
3. **Restart IYARI session** to pick up the new MCP server
