...
| Version | `2.0.0` |
| Author | Francesco Bonacci (f-trycua), IYARI |
| License | MIT |
...
IYARI drives [cua-driver](https://github.com/trycua/cua) under the hood.
This wrapper skill teaches the IYARI `computer_use` workflow and action
...
skill installed by `cua-driver skills install`. IYARI autodetection is a
planned cua-driver follow-up, so currently point IYARI at the resulting
...
1. **Config grant (standard and unrestricted modes).** When
   ...
   IYARI applies the same host-side floor in unrestricted mode. If it is not
...
Explicit IYARI YOLO (`--yolo`, `/yolo`, or `approvals.mode: off`) launches an
...
IYARI intentionally keeps THIS skill focused on the IYARI-side
...
IYARI autodetection is a planned follow-up in trycua/cua. For now, the command
installs the pack under `~/.cua-driver/skills/cua-driver`; point IYARI at that
