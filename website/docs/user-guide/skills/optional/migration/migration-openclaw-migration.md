---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into IYARI"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into IYARI"
---
...
Import an OpenClaw setup (memories, skills) into IYARI.
...
| Tags | `Migration`, `OpenClaw`, `IYARI`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> IYARI Migration

Use this skill when a user wants to move their OpenClaw setup into IYARI with minimal manual cleanup.
...
- import `SOUL.md` into the IYARI home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into IYARI memory entries
- merge OpenClaw command approval patterns into IYARI `command_allowlist`
- migrate IYARI-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to IYARI working-directory configuration
- copy OpenClaw skills into `~/.hermes/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen IYARI workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct IYARI destination
...
With `--migrate-secrets`, it will also import a small allowlisted set of IYARI-compatible secrets, currently:
...
IYARI CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in IYARI. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a IYARI workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing IYARI skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of IYARI unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty IYARI targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean IYARI destination. Unsupported auth blobs must still be reported as skipped.
...
- IYARI persona state imported
- IYARI memory files populated with converted OpenClaw knowledge
