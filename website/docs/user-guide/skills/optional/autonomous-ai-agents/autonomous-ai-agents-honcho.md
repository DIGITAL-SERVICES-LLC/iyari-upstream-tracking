---
title: "Honcho — Configure and troubleshoot Honcho memory for IYARI"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for IYARI"
---
...
Configure and troubleshoot Honcho memory for IYARI.
...
# Honcho Memory for IYARI

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every IYARI profile its own peer identity while sharing a unified view of the user.
...
3. **AI peer card** -- the identity card for this IYARI profile's AI peer
...
Honcho models conversations as interactions between **peers**. IYARI creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this IYARI instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each IYARI run |
...
Each IYARI profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
Guidelines for IYARI when Honcho memory is active.
...
| `hermes honcho sync` | Create host blocks for all IYARI profiles that don't have one yet |
| `hermes honcho migrate` | Step-by-step migration guide from OpenClaw native memory to IYARI + Honcho |
