...
| Version | `0.1.1` |
| Author | Matt Maximo (MattMaximo), IYARI |
| License | MIT |
...
# Grok Build CLI — IYARI Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the IYARI terminal. Grok
...
1. Prepare stable input files first with IYARI tools (`read_file`,
...
2. **Don't conflate IYARI's xAI auth with the `grok` CLI's auth.** IYARI's
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume IYARI's xAI auth carries
   over.
