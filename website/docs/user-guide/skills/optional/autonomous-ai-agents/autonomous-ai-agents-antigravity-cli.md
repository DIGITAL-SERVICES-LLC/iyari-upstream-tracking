...
| Version | `0.2.0` |
| Author | Tony Simons (asimons81), IYARI |
| License | MIT |
...
commands through the IYARI `terminal` tool; inspect its config and logs with
`read_file`. This skill is reference + procedure — it does not wrap a network
API, so there is nothing to authenticate from IYARI itself.
