---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in IYARI"
sidebar_label: "Actual Setup"
description: "Set up Actual Computer (actual.inc) inference in IYARI"
---
...
Set up Actual Computer (actual.inc) inference in IYARI.
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a IYARI inference
...
- User has an `ac_` key and wants IYARI routed through their Actual cluster.
...
- IYARI has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current IYARI — the
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point IYARI at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
...
1. **reasoning_effort trap (handled by IYARI since the first-class provider).**
   ...
   request still 400s this way on an old IYARI, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** IYARI's default toolset
   ...
   servers emit a bare `data: [DONE]` — IYARI reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current IYARI the built-in provider wins the name; stale custom blocks
