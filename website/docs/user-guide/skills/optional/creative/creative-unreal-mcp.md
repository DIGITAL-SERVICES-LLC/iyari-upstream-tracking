...
Companion skill for the `unreal-engine` entry in the IYARI MCP catalog. The
...
Two halves, in this order: the editor side must be up before IYARI connects.
...
Do NOT use `ModelContextProtocol.GenerateClientConfig` for IYARI — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. IYARI connects
...
2. Start the IYARI session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through IYARI
they appear as:
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — IYARI runs batched calls
...
- **Start order matters.** Editor + server up first, then the IYARI
...
- **Timeouts on long operations.** IYARI's per-call default is 120 s; asset
