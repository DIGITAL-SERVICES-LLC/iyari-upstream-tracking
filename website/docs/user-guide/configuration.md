...
- `hermes-profile=<sanitized profile name>` — scopes reuse and reaping to the active IYARI profile

On startup, IYARI runs `docker ps --filter label=hermes-task-id=<id> --filter label=hermes-profile=<profile>` and **attaches to the existing container** when it finds one. If the container is `exited` (e.g. after a Docker daemon restart), it's `docker start`'d and reused — filesystem state and any installed packages survive, but in-container background processes do not.
...
If the requested provider is unavailable, IYARI falls back automatically in this order: `local` → `groq` → `openai`.
