...
sidebar_position: 1
title: "CLI Interface"
description: "Master the IYARI terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

IYARI's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.
...
:::tip
IYARI also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `hermes --tui` — see the [TUI](tui.md) guide.
:::
...
The `hermes plugins` commands manage native IYARI plugins and portable Agent
...
Portable packages remain disabled until explicitly enabled. IYARI currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the IYARI CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The IYARI CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend IYARI to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. IYARI supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for IYARI replies |
...
IYARI loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.
...
personalities:
  ...
  pirate: "Arrr! Ye be talkin' to Captain IYARI..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, IYARI also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches IYARI — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while IYARI is working, IYARI prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend IYARI to the background — just like any terminal process. The shell prints a confirmation:

```
IYARI has been suspended. Run `fg` to bring IYARI back.
```
...
CLI sessions are stored in IYARI's SQLite state database under `~/.hermes/state.db`. The database keeps:
...
IYARI immediately confirms the task and gives you back the prompt:
