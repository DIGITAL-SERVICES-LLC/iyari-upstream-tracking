...
IYARI runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. IYARI
...
1. IYARI captures a baseline of current diagnostics for the file.
...
…). IYARI auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point IYARI at the extracted bundle — the directory that contains
...
`node_modules` tree — IYARI installs both packages together when you
...
When `install_strategy: auto`, IYARI installs binaries into
...
shared location — the staging dir is fully IYARI-owned and is
