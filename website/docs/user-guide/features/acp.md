...
description: "Use IYARI inside ACP-compatible editors and collaboration platforms"
---

# ACP Host Integration

IYARI can run as an ACP server, letting ACP-compatible hosts talk to
IYARI over stdio. Editors can render:
...
IYARI. ACP is a good fit when you want IYARI to keep its existing identity,
provider setup, memory, skills, and tools while another application owns the
conversation transport.

## What IYARI exposes in ACP mode

IYARI runs with a curated `hermes-acp` toolset designed for editor workflows. It includes:
...
Install IYARI normally, then add the ACP extra from the install checkout:
...
Any of the following starts IYARI in ACP mode:
...
IYARI logs to stderr so stdout remains reserved for ACP JSON-RPC traffic.
...
- `npm install -g agent-browser @askjo/camofox-browser` into that prefix (no sudo needed — `npm`'s `--prefix` points at the user-writable IYARI-managed Node)
...
```text
Buzz relay <-- WebSocket --> buzz-acp <-- ACP over stdio --> IYARI
```

This is a transport integration, not a second IYARI installation. The
subprocess launched by `buzz-acp` uses the same IYARI configuration,
...
spawns IYARI locally as a preset harness. The relay bridge is for joining Buzz
...
- Mint a dedicated Nostr keypair for IYARI (`buzz-admin generate-key`) and
...
the same operating-system user that owns the intended IYARI home. Setup,
...
The bridge discovers every Buzz channel where the IYARI identity is a member
and automatically subscribes when it is added to another channel. Buzz channel
membership therefore remains the access boundary; IYARI does not need a
separate channel list in its own configuration.

To expose IYARI ACP activity in the owner's Buzz Desktop, add:
...
where IYARI is expected to work.
...
2. Select **IYARI** from the built-in agent list.
3. Connect and start chatting.

If you want to define IYARI manually, add it through VS Code settings under `acp.agents`:
...
Configure IYARI as a custom agent server in Zed settings:
...
3. Start a new IYARI external-agent thread.

Prerequisites:

- Configure IYARI provider credentials first with `hermes model`, or set them in `~/.hermes/.env` / `~/.hermes/config.yaml`.
...
[Buzz](https://github.com/block/buzz) ships IYARI as a preset runtime.
With IYARI installed the normal way, Buzz discovers it automatically —
open **Settings → Runtimes** and IYARI appears under your runtimes.
...
Buzz Desktop (v0.5.1+) renders IYARI's full model menu in the agent's runtime
settings. The list comes from IYARI itself over ACP: it shows every model
from providers you have authenticated in IYARI (the same inventory behind
`hermes model` and the `/model` command), so a model missing from the menu
means its provider has no credentials configured on the IYARI side.
...
change your IYARI-wide default — use `hermes model` for that.

#### Keep Buzz agents owner-only

Buzz creates every agent with **Who can talk to this agent** set to `Owner only`.
Leave it there when the runtime is IYARI.

Two behaviors combine on this path. The `hermes-acp` toolset includes `terminal`
and `execute_code`, and Buzz's ACP bridge answers IYARI's permission requests
itself with `allow_once` rather than surfacing them. A IYARI agent in Buzz
...
- `approvals.mode: manual` does make IYARI raise the permission request, but
...
ACP mode uses the same IYARI configuration as the CLI:
...
Provider resolution uses IYARI's normal runtime resolver, so ACP inherits the currently configured provider and credentials. IYARI also advertises a terminal auth method (`--setup`) for first-run ACP clients; this opens IYARI's interactive model/provider setup.

## Host integration

These variables are set by an **ACP host process** (an editor or another agent
harness) on the IYARI subprocess it spawns. They are not user configuration —
...
IYARI normally starts every MCP server configured in `config.yaml` before it
...
The underlying `AIAgent` still uses IYARI's normal persistence/logging paths, but ACP `list/load/resume/fork` are scoped to the currently running ACP server process.

## Working directory behavior

ACP sessions bind the editor's cwd to the IYARI task ID so file and terminal tools run relative to the editor workspace, not the server process cwd.
...
| `allow_always` | Allow always | All future sessions | Yes (written to the IYARI permanent allowlist) |
| `deny` | Deny | This one tool call | No |

`allow_session` is the right default for an editor workflow where you trust an agent for the duration of a task but don't want to grant a long-lived allowlist entry. The safety trade-off is straightforward: the broader the scope, the less the editor will interrupt you, and the more damage a misbehaving agent (or prompt injection) can do before you notice. Start with `allow_once` for unfamiliar commands; promote to `allow_session` once you've seen the agent run the same pattern correctly a few times; reserve `allow_always` for truly idempotent commands you trust forever (e.g. `git status`).

The ACP bridge maps these options onto IYARI's internal approval semantics — `allow_always` writes a permanent allowlist entry the same way the CLI does, while `allow_session` only affects the in-process approval cache for the current ACP session.
...
- IYARI is installed and on your PATH.
...
ACP mode uses IYARI's existing provider setup. Configure credentials with:
