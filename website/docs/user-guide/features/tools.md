...
description: "Overview of IYARI's tools — what's available, how toolsets work, and terminal backends"
---
...
IYARI ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, scheduled tasks, Home Assistant, and more.
...
**One persistent container, shared across the whole process.** IYARI starts a single long-lived container on first use (`docker run -d ... sleep infinity`) and routes every terminal, file, and `execute_code` call through `docker exec` into that same container. Working-directory changes, installed packages, environment tweaks, and files written to `/workspace` all carry over from one tool call to the next, across `/new`, `/reset`, and `delegate_task` subagents, for the lifetime of the IYARI process. The container is stopped and removed on shutdown.
...
Authenticate with all three of `VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, and `VERCEL_TEAM_ID`. This access-token setup is the supported path for deployments and normal long-running IYARI processes on Render, Railway, Docker, and similar hosts. Supported runtimes are `node24`, `node22`, and `python3.13`; IYARI defaults to `/vercel/sandbox` as the remote workspace root.
...
With `container_persistent: true`, IYARI uses Vercel snapshots to preserve filesystem state across sandbox recreation for the same task. This can include IYARI-synced credentials, skills, and cache files inside the sandbox. Snapshots do not preserve live processes, PID space, or the same live sandbox identity.

Background terminal commands use IYARI's generic non-local process flow: spawn, poll, wait, log, and kill work through the normal process tool while the sandbox is alive, but IYARI does not provide native Vercel detached-process recovery after cleanup or restart.
