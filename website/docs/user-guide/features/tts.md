...
Each provider has a documented per-request input-character cap. IYARI truncates text before calling the provider so requests never fail with a length error:
...
If your configured provider isn't available, IYARI automatically falls back:
