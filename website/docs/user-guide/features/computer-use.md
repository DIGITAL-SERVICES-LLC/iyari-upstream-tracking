...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or IYARI app). `hermes computer-use doctor` will tell you which permission is missing. |
...
IYARI maps its existing approval UX onto cua-driver 0.10's immutable daemon
...
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal IYARI approvals; Cua stops at its protected boundary | Refuses unless a certified protected host is available; IYARI does not claim one today |
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit IYARI risk acceptance; no runtime Cua prompts | Allowed within Cua's built-in, managed, and user policy ceilings |

The unrestricted daemon is private to that IYARI session. Turning `/yolo`
...
daemon's mode or grants another IYARI conversation the same authority.
...
agent is acting here." Each IYARI run declares its own cua-driver
...
IYARI intentionally keeps its skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the IYARI-side `computer_use` action vocabulary — the
...
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects IYARI (planned follow-up), this happens
