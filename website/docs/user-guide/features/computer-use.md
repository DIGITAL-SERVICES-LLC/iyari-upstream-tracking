...
IYARI can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended IYARI integration. It
...
**Fresh installs already have the driver.** The IYARI installer
...
Already have cua-driver? IYARI reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `hermes update`, and the first
`computer_use` call of a session, IYARI checks the local version and
...
under your control, so IYARI reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. IYARI autodetection is a
planned cua-driver follow-up, so currently point IYARI at that directory or
...
interface. The built-in toolset provides IYARI actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `hermes computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the IYARI host identity. |
...
IYARI maps its existing approval UX onto cua-driver's immutable runtime
...
| IYARI session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal IYARI approvals; Cua stops at its protected boundary | Refuses unless `computer_use.grant_existing_profile: true` (one-time config opt-in) |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit IYARI risk acceptance; no runtime Cua prompts | Refuses unless `computer_use.grant_existing_profile: true`; YOLO does not substitute for this grant |
...
IYARI then launches the cua-driver runtime with the trusted-launcher grant
...
for the format). IYARI launches a private runtime with
...
resetting or closing the IYARI session, cancellation cleanup, or process exit
closes that transport session. IYARI also stops private runtimes that it
launched for bounded, unrestricted, or existing-profile access. One IYARI
...
embedded service under the IYARI host identity.
...
IYARI run declares a public cua-driver **session name** (something like
...
IYARI keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the IYARI-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. IYARI
autodetection is a planned cua-driver follow-up, so currently point IYARI at
...
These are **platform deep dives, not duplicates of the IYARI skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **IYARI
...
image capture also saves a bounded, shareable copy under IYARI's image cache
...
IYARI applies multi-layer guardrails:
...
Screenshots are expensive. IYARI applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  IYARI agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the IYARI agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. IYARI warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive IYARI from inside the RDP / console
...
upstream. **IYARI disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) IYARI sets
...
unreleased fix — point IYARI at a binary you built from source instead
of the published release. IYARI resolves the driver with
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for IYARI testing (see notes).
...
### Option B — point IYARI straight at the built binary (fastest loop)
...
### Confirm IYARI is using your build
...
- **IYARI spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  IYARI YOLO, IYARI instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no IYARI
  change needed either way.
- **Local builds skip the version check.** IYARI warns when the
...
- **IYARI-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  IYARI `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. IYARI autodetection is a
  planned follow-up; currently point IYARI at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (IYARI among them)
