...
description: "Re-run a prompt on a recurring interval inside your session — IYARI's take on Claude Code's /loop."
---

# Recurring Loops (`/loop`)

`/loop` re-runs a prompt (or a slash command) on a recurring cadence **inside your current session**. Each wakeup is a real agent turn: IYARI reads the current state fresh — the latest CI result, the newest queue depth, the file as it is now — does the work, reports back, and goes quiet until the next tick.

It's IYARI's take on **Claude Code's `/loop`** (and its `/proactive` alias, which works here too). Where [`/goal`](./goals.md) is judge-driven — "keep working until this objective is achieved" — `/loop` is timer-driven: "do this again every N minutes (or whenever it makes sense) until something says stop."
...
2. **First wakeup in 5m** — while the session is idle, IYARI injects the wakeup and runs a normal turn against current state.
