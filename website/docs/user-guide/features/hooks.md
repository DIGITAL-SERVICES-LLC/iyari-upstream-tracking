...
IYARI has four hook systems that run custom code at key lifecycle points:
...
This tutorial shows how to build it yourself as a user-defined hook. IYARI does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.
...
An earlier version of IYARI shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.
...
| `session_id` | `str` | Current IYARI session id |
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is IYARI's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.

The clean user-message `content` remains unchanged. For replay and prompt-cache stability, IYARI may persist the exact API-bound message, including plugin-injected context, in the row's `api_content` sidecar.
...
```python
VOCAB = "IYARI, Teknium, Nous Research, kanban"
...
Declare shell-script hooks in your `~/.hermes/config.yaml` and IYARI will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Each time the event fires, IYARI spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // IYARI-canonical

// Modify a pre_tool_call — rewrite tool args before dispatch:
{"action": "modify", "args": {"new_string": "fixed content"}}         // IYARI-canonical
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate IYARI event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
Each unique `(event, command)` pair prompts the user for approval the first time IYARI sees it, then persists the decision to `~/.hermes/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.
...
Outbound webhooks are the push-side mirror of the [inbound webhook platform](/user-guide/messaging/webhooks): inbound webhooks wake IYARI when the world changes; outbound webhooks tell the world when IYARI does something. Configure a list of HTTP endpoints and the lifecycle events they care about, and IYARI POSTs a signed JSON payload to each endpoint whenever a matching event fires — no polling on the receiving end.
...
- Wake *another* IYARI instance: point the URL at that instance's inbound webhook
...
| `X-IYARI-Event` | The hook event name |
| `X-IYARI-Delivery` | Unique id per delivery — same value as `delivery_id` in the body |
| `X-IYARI-Signature-256` | `sha256=<hex>` — HMAC-SHA256 of the raw body, GitHub-style; only present when a secret is configured |
...
- **Dedupe** on `delivery_id` (or the matching `X-IYARI-Delivery` header) — remember recently seen ids and skip duplicates. IYARI retries failed deliveries once, so the same id can legitimately arrive twice.
