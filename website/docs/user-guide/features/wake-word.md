...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey IYARI's wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey IYARI")

The wake word turns IYARI into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, IYARI listens in the background for a spoken
trigger phrase. Say it, and IYARI starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** IYARI backend (for example a
...
IYARI supports **client capture** for that case:
...
By default the phrase is **"hey hermes"** — a model for it ships with IYARI, so
...
default) to let IYARI choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which IYARI installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three IYARI surfaces, and `surface` picks which one
...
including when IYARI surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. IYARI does not silently fail over to another open surface.
...
"Hey IYARI" works out of the box — the bundled openWakeWord model
...
Create a "Hey IYARI" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
but the phrase never fires. IYARI detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the IYARI
backend (it may appear as your terminal, `python`, or IYARI), then toggle the
