...
title: "Plugins"
description: "Extend IYARI with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

IYARI has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in IYARI
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a IYARI Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.
...
Start IYARI — your tools appear alongside built-in tools. The model can call them immediately.
...
```python
"""Minimal IYARI plugin — registers a tool and a hook."""
...
Drop both files into `~/.hermes/plugins/hello-world/`, restart IYARI, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but IYARI does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.hermes/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting IYARI.
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with IYARI — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
...
Within each source, IYARI also recognizes sub-category directories that route plugins to specialized discovery systems:
...
IYARI checks out the commit detached, verifies that `HEAD` exactly matches the
...
Several categories of plugin bypass `plugins.enabled` — they're part of IYARI's built-in surface and would break basic functionality if gated off by default:
...
IYARI tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. IYARI runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, IYARI never
materializes the prompt on another surface.

IYARI still owns hardline blocks, sudo-stdin protection, user deny rules,
...
IYARI has four kinds of plugins:
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and IYARI also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a IYARI Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a IYARI Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a IYARI Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. IYARI auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
...
IYARI Desktop registers the `hermes://` URL scheme, so a website, README, or
...
Clicking one opens IYARI and shows a **confirmation dialog** — the repo id,
...
```html
<a href="hermes://plugin/install?repo=owner/repo&enable=1">Install in IYARI</a>
```

MCP servers have the equivalent link form — see
[Add to IYARI link](/reference/mcp-config-reference#add-to-hermes-link).
...
not a code audit, and IYARI has not reviewed the plugin's code. Only install
...
all a bundled seed copy ships with IYARI — so search works fully offline.
...
- IYARI reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- IYARI rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless IYARI can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. IYARI drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. IYARI checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through IYARI's existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
