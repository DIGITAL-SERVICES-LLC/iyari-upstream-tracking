...
**Tool Search** is IYARI's opt-in progressive-disclosure layer for that
...
:::info Built-in IYARI tools never defer
The tools that make up IYARI's core capability set (`terminal`,
...
When the model invokes `tool_call`, IYARI **unwraps the bridge** and
...
- **No JS sandbox.** IYARI uses the simpler "structured tools" mode
