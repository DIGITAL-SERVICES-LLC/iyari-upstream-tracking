...
description: "How IYARI remembers across sessions — MEMORY.md, USER.md, and session search"
---

# Persistent Memory

IYARI has bounded, curated memory that persists across sessions. This lets it remember your preferences, your projects, your environment, and things it has learned.
...
:::caution One agent per IYARI home
Don't point two agent processes at the same IYARI home directory. Memory writes are automatic and load back into the system prompt at session start, so two writers sharing one home will compound each other's entries into state neither of them (nor you) authored. Memory is scoped per [profile](/user-guide/profiles) by design — give a second agent its own profile, and if they need shared memory, use an [external memory provider](/user-guide/features/memory-providers) instead.
:::
...
The learning journey is a timeline view of everything IYARI has learned — saved skills and memory entries plotted over time (oldest at top, newest at bottom), with a playable "constellation" scrubber that replays the build-up. The same graph data drives three surfaces:
...
Beyond viewing, the journey is also where you **prune and correct** what IYARI has learned:
...
or update a skill. This is IYARI's consent-aware learning loop: repeated
...
For deeper, persistent memory that goes beyond MEMORY.md and USER.md, IYARI ships with 8 external memory provider plugins — including Honcho, OpenViking, Mem0, Hindsight, Holographic, RetainDB, ByteRover, and Supermemory.
