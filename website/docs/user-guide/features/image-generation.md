...
IYARI generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `hermes tools` and persists in `config.yaml`.
...
`max_parallel_requests` defaults to `4`. IYARI clamps it to at least one and
...
new models appear in the picker as soon as OpenRouter serves them; no IYARI
...
as a tool the chat model may call, and IYARI cannot force the call — the
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; IYARI materializes them to the local cache so delivery still works after expiry
