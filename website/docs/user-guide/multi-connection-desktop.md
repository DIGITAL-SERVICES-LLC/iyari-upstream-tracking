...
# Connecting Desktop to Many IYARI Instances

Register every IYARI backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and IYARI Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another IYARI gateway…"**) deep-links straight to
...
list of IYARI gateways. Its intro says it plainly: *"Manage this device and
every IYARI gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The IYARI runtime managed by this app." | automatic |
| **Remote gateway** | "A IYARI gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A IYARI install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **IYARI Cloud** | "A hosted instance discovered through your IYARI Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the IYARI Cloud sign-in/discovery flow at
  the top of the Gateways page — the **IYARI Cloud** kind in the add-connection
...
3. Pick the kind: **Local**, **IYARI Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **IYARI Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
...
Gateways are machines or hosted backends; profiles are isolated IYARI agents
...
- **IYARI Cloud** instances are skipped with a *"Managed by IYARI Cloud"*
...
panel, ⌘K **Update IYARI**, the update-ready toast) run the same fan-out
...
- **"Update IYARI Desktop to chat with agents on other connections"** — the
