...
def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default IYARI root for the guard.
...
#: is OURS: it declares "this process tree is running under IYARI test
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real IYARI home *root*.
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real IYARI root {root}). "
...
def _wal_reset_repair_hint() -> str:
    ...
    try:
        ...
        if method in {"git", "unknown"}:
            return f"IYARI-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart IYARI"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a IYARI-managed install (uv-managed venv)
...
# threads inside ONE interpreter, yet a normal IYARI host runs several
...
# included because rollback-journal (DELETE) mode — IYARI's fallback on
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the IYARI home tree (``get_hermes_home()``) — the safe repair scope:
      IYARI owns those files, and the OS makes ``chmod`` fail on files the
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. IYARI needs read-write access to open the database. "
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, IYARI deliberately runs ``state.db`` in
...
def _looks_like_hermes(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a IYARI process?
    ...
    We only flag processes that look like IYARI, not every system daemon.
    """
    lower = cmdline.lower()
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # legitimately held for multi-second stretches by sibling IYARI
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another IYARI process held the "
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a IYARI process —
                        # only flag uninspectable holders that look like
                        # another IYARI instance, not every system daemon.
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade IYARI, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one IYARI session.

        A IYARI session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a IYARI session is already bound to any Telegram DM topic.
