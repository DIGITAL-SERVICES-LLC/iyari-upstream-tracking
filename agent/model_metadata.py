...
# Minimum context length required to run IYARI.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent IYARI from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for IYARI's system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets IYARI minimum.
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # IYARI grow conversations past the runtime limit and Ollama
...
# who never asked for it. The variant suffix is a IYARI-side alias: it is
...
# IYARI-side picker suffix that opts a Codex slug into the live-verified
...
def strip_codex_context_variant_suffix(model: Optional[str]) -> str:
    ...
    The suffix is a IYARI picker alias (``gpt-5.6-sol-900k``); the Codex
...
def _resolve_codex_oauth_context_length_with_source(
    ...
    # ``-900k`` variants are IYARI picker aliases — the Codex catalog only
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets IYARI sends to providers:
