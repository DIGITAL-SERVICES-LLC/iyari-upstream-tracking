...
Owns one Codex thread per IYARI session. Drives `turn/start`, consumes
...
# IYARI's tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse IYARI/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per IYARI session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but IYARI's text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into IYARI's approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved IYARI's tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO IYARI
        approval-mode/timeout logic. The IYARI-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; IYARI approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map IYARI approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    IYARI returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the IYARI approval mode/timeout resolution
...
def _get_hermes_version() -> str:
    """Best-effort IYARI version string for codex's userAgent line."""
    try:
