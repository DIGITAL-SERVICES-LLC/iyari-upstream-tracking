"""IYARI-tools-as-MCP server for the codex_app_server runtime.

When the user runs `openai/*` turns through the codex app-server, codex
owns the loop and builds its own tool list. By default, that means
IYARI's richer tool surface — web search, browser automation,
delegate_task subagents, vision analysis, persistent memory, skills,
cross-session search, image generation, TTS — is unreachable.

This module exposes a curated subset of those IYARI tools to the
spawned codex subprocess via stdio MCP. Codex registers it as a normal
MCP server (per `~/.codex/config.toml [mcp_servers.hermes-tools]`) and
the user gets full IYARI capability inside a Codex turn.
...
Scope (what we expose):
  ...
  - skill_view, skills_list              — IYARI's skill library
  - text_to_speech                       — TTS
...
What we DO NOT expose:
  ...
  - delegate_task / memory /             — `_AGENT_LOOP_TOOLS` in IYARI
...
# Tools we expose. Each name MUST match a registered IYARI tool that
...
#     `_AGENT_LOOP_TOOLS` in IYARI (model_tools.py:493). They require
#     the running AIAgent context to dispatch (mid-loop state), so a
#     stateless MCP callback can't drive them. IYARI's default runtime
...
def _build_server() -> Any:
    """Create the MCP server with IYARI tools attached. Lazy imports
    ...
    # Discover IYARI tools so dispatch works.
    ...
    mcp = MCPServer(
        "hermes-tools",
        instructions=(
            "IYARI's tool surface, exposed for use inside a Codex "
    ...
    # Pull authoritative IYARI tool schemas for the ones we expose, so
    # MCP clients see the same parameter docs IYARI gives the model.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug(
                "skipping %s — not registered in this IYARI process", name
            )
            continue

        description = spec.get("description") or f"IYARI {name} tool"
        ...
        # string, and carries a __signature__ synthesized from the IYARI
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # Quiet mode: keep IYARI's own banners off stdout (which is the MCP wire).
