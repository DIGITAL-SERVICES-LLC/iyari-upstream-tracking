...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # IYARI owns auxiliary retry + provider/model fallback policy (the
    ...
    # IYARI sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let IYARI control the budget; explicit callers can still
    # override via kwargs.
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before IYARI triggers summarization.  Higher values delay
...
def _codex_cloudflare_headers(
    ...
    the official endpoint always send IYARI's originator and version. Custom
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (IYARI)",
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the IYARI-side ``-900k`` large-context picker suffix —
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private IYARI plumbing (_reasoning_config
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from IYARI auth store.
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into IYARI's built-in discovery
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    # See _create_openai_client: disable SDK-internal retries so IYARI owns
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate IYARI reasoning into native
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional IYARI reasoning config for direct model calls
