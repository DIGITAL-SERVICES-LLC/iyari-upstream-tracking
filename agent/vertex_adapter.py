"""Vertex AI (Google Cloud) adapter for IYARI.

Provides authentication and configuration for Vertex AI's OpenAI-compatible
endpoint. This allows IYARI to use Gemini models via Google Cloud with
...
def has_explicit_vertex_config() -> bool:
    """True only when the user deliberately pointed IYARI at Vertex.
    ...
    IYARI up for Vertex would suddenly see it (and could spend against those
    credentials). So this checks only IYARI-scoped signals:
      ...
      * a resolvable ``VERTEX_CREDENTIALS_PATH`` service-account file
        (the IYARI-specific path var — NOT ``GOOGLE_APPLICATION_CREDENTIALS``).
    """
    if _resolve_project_override():
        return True
