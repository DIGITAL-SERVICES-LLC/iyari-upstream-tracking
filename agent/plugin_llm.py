...
Plugins built on IYARI often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing IYARI
...
embedded slugs, no shorthands. This mirrors IYARI's main config
...
handles every provider, fallback chain, and per-task override IYARI
supports.
"""
