...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.hermes/.env``. See digital-services-llc/iyari#2765.
...
def _refuse_checkpoint_required_on_codex_app_server(
    ...
    the default — IYARI never even initiates the compaction), so no
...
def init_agent(
    ...
    # management API (the historical IYARI behavior) or left to LM Studio's
    ...
    # Lets an enterprise admin append to or replace IYARI's built-in
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by IYARI.  Choose a model with at least "
