"""Anthropic Messages API adapter for IYARI.

Translates between IYARI's internal OpenAI-style message format and
...
# IYARI effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    # from ``ANTHROPIC_API_KEY`` (IYARI loads that into the process env from
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    IYARI notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    IYARI historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by IYARI)
    ...
    # 1. IYARI-managed OAuth/setup token env var
    ...
    # 4. IYARI credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration IYARI configs.
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run IYARI-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize IYARI with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read IYARI-managed OAuth credentials from ~/.hermes/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read IYARI OAuth credentials: %s", e)
    return None
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("IYARI", "Claude Code")
                text = text.replace("IYARI agent", "Claude Code")
                text = text.replace("hermes-agent", "claude-code")
                text = text.replace("Digital Services LLC", "Anthropic")
        ...
        #      a) bare IYARI-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that IYARI surfaces in its CLI. We
