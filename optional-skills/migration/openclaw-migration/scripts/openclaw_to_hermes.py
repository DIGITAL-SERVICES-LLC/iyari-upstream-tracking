#!/usr/bin/env python3
"""OpenClaw -> IYARI migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into IYARI, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into IYARI.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into IYARI memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into IYARI memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import IYARI-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of IYARI-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into IYARI command_allowlist.",
    },
    "skills": {
        "label": "User skills",
    ...
    "discord-settings": {
        ...
        "description": "Import Discord bot token and allowlist into IYARI .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into IYARI .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into IYARI .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into IYARI .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into IYARI .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into IYARI config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into IYARI config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into IYARI.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into IYARI MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into IYARI config.yaml.",
    },
    "plugins-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into IYARI config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into IYARI session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into IYARI custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into IYARI config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into IYARI config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into IYARI config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    """
    if yaml is None:
        raise RuntimeError("PyYAML is required to update IYARI config.yaml")
    ensure_parent(path)
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with IYARI in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving, but NOT a simple .lower() of one replacement: capitalized/
# mixed-case matches → ``IYARI`` (prose, the product name), while all-lowercase
# matches → ``hermes`` (so filesystem paths like ``~/.openclaw`` become the
# REAL protected path ``~/.hermes`` — not the broken ``~/.iyari``, which was
# never a real command/path convention). ``IYARI`` and ``hermes`` are
# different words, not a case variant of each other, so both are spelled out
# explicitly rather than derived via ``.lower()``.
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'IYARI', 'hermes'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'IYARI', 'hermes'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'IYARI', 'hermes'),
]


def _case_preserving_replacement(replacement: str, lowercase_replacement: str):
    """Return a re.sub replacement fn that swaps in *lowercase_replacement*
    when the matched text was all-lowercase, *replacement* otherwise.

    Keeps ``OpenClaw`` → ``IYARI`` but maps ``openclaw`` → ``hermes`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.hermes/config.yaml`` (the real IYARI home) instead of the broken
    ``~/.iyari/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with IYARI.
    ...
    for pattern, replacement, lowercase_replacement in _REBRAND_PATTERNS:
        text = pattern.sub(
            _case_preserving_replacement(replacement, lowercase_replacement), text
        )
    return text


def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION IYARI memory store (memories/MEMORY.md, USER.md).
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> IYARI Migration Report",
        "",
...
class Migrator:
    ...
    # Option ids that mutate the IYARI config.yaml file.  Once any one of
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new IYARI session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "IYARI config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the IYARI
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No IYARI-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted IYARI-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; IYARI still uses "edge"
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct IYARI destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected IYARI-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected IYARI-compatible values were extracted; raw credentials file was not copied."),
        ]
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in IYARI config")
                continue
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> IYARI reasoning_effort
        ...
        if changes:
            ...
            self.record("agent-config", "openclaw.json agents.defaults", "config.yaml agent/compression/terminal",
                        "migrated", "Agent defaults mapped to IYARI config")
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to IYARI config
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map fields that have IYARI equivalents
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> IYARI Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in IYARI. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing IYARI Config Not Overwritten)",
                "",
                "These items already existed in your IYARI config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the IYARI agent discovers these",
            "directories, it may read/write to them instead of the IYARI state, causing",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into IYARI.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("HERMES_HOME") or str(Path.home() / ".hermes"), help="IYARI home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing IYARI targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of IYARI-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> IYARI Migration   [{mode_label:>8s}]   ║")
    print("  ╠══════════════════════════════════════════════════════╣")
